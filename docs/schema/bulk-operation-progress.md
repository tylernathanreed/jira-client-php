...
| --- | --- | --- |
| `created` | `` | A timestamp of when the task was submitted. |
...
| `invalidOrInaccessibleIssueCount` | `` | The number of issues that are either invalid or issues that the user doesn't have permission to view, regardless of the success or failure of the operation. |
...
| `progressPercent` | `` | Progress of the task as a percentage. |
| `started` | `` | A timestamp of when the task was started. |
| `status` | `'ENQUEUED'|'RUNNING'|'COMPLETE'|'FAILED'|'CANCEL_REQUESTED'|'CANCELLED'|'DEAD'|null` | The status of the task. |
| `submittedBy` | `` |  |
| `taskId` | `` | The ID of the task. |
| `totalIssueCount` | `` | The number of issues that the bulk operation was attempted on. |
| `updated` | `` | A timestamp of when the task progress was last updated. |
