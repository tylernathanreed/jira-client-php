...
| `editPermissions` | [`?list<SharePermission>`](/docs/schemas/share-permission.md) | The groups and projects that can edit the filter. This can be specified when updating a filter, but not when creating a filter. |
...
| `sharePermissions` | [`?list<SharePermission>`](/docs/schemas/share-permission.md) | The groups and projects that the filter is shared with. This can be specified when updating a filter, but not when creating a filter. |
| `subscriptions` | [`?list<FilterSubscription>`](/docs/schemas/filter-subscription.md) | The users that are subscribed to the filter. |
