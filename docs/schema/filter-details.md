...
| `editPermissions` | [`?list<SharePermission>`](/docs/schema/share-permission.md) | The groups and projects that can edit the filter. This can be specified when updating a filter, but not when creating a filter. |
...
| `subscriptions` | [`?list<FilterSubscription>`](/docs/schema/filter-subscription.md) | The users that are subscribed to the filter. |
