...
| `fieldOptions` | `array` | A list of options related to the field, applicable in contexts where multiple selections are allowed. |
| `id` | `string` | The unique ID of the field. |
| `isRequired` | `bool` | Indicates whether the field is mandatory for the operation. |
| `multiSelectFieldOptions` | `array` | Specifies supported actions (like add, replace, remove) on multi-select fields via an enum. |
